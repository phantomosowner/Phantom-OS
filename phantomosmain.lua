if IY_LOADED and not _G.pos_DEBUG == true then
	-- error("PhantomOS is already running!", 0)
	return
end

pcall(function() getgenv().pos_LOADED = true end)

COREGUI = game:GetService("CoreGui")
if not game:IsLoaded() then
	local notLoaded = Instance.new("Message")
	notLoaded.Parent = COREGUI
	notLoaded.Text = 'PhantomOS is waiting for the game to load'
	game.Loaded:Wait()
	notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "PhantomOS v" .. currentVersion
...
Logo.Image = "rbxassetid://16538343305"
...
About.TextSize = 14
About.Text = "Plugins are .pos files and should be located in the 'workspace' folder of your exploit."
...
PluginsHint.Text = "Plugins Are currently not supported :("
...
reference = (function()
	local main = create({
		...
		{113,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=4,Name="Header",Parent={112},Position=UDim2.new(0,8,0,5),Size=UDim2.new(1,-8,0,20),Text="Join community",TextColor3=Color3.new(1,1,1),TextSize=20,TextXAlignment=0,ZIndex=10,}},
		{114,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Text",Parent={112},Position=UDim2.new(0,8,0,30),Size=UDim2.new(1,-8,0,32),Text="You can join the discord for help/fun!.",TextColor3=Color3.new(1,1,1),TextSize=14,TextWrapped=true,TextXAlignment=0,ZIndex=10,}},
	...
	inviteButton.MouseButton1Click:Connect(function()
		...
		if func then
			func("https://discord.gg/Sa5p7qgYXp")
...
function saves()
	if writefileExploit() then
		if pcall(function() readfile("IY_FE.pos") end) then
			if readfile("IY_FE.pos") ~= nil then
				local success, response = pcall(function()
					local json = HttpService:JSONDecode(readfile("IY_FE.pos"))
				...
				if not success then
					...
					writefileCooldown("IY_FE.pos", defaults)
					wait()
					saves()
				end
			else
...
function updatesaves()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("IY_FE.pos", HttpService:JSONEncode(update))
	end
end
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- PhantomOS Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the PhantomOS support server.'}
			...
			if cmd then
				...
				if infTimes then
					while lastBreakTime < cmdStartTime do
						...
						if not success and _G.pos_DEBUG then
...
function addPlugin(name)
	if name:lower() == 'plugin file name' or name:lower() == 'iy_fe.pos' or name == 'iy_fe' then
		notify('Plugin Error','Please enter a valid plugin')
	else
		local file
		local fileName
		if name:sub(-3) == '.pos' then
			pcall(function() file = readfile(name) end)
			fileName = name
		else
			pcall(function() file = readfile(name..'.pos') end)
			fileName = name..'.pos'
		end
		if file then
...
task.spawn(function()
	local success, latestVersionInfo = pcall(function() 
		local versionJson = game:HttpGet('https://pastebin.com/raw/WHQDPPuN')
		return HttpService:JSONDecode(versionJson)
	end)

	if success then
		if currentVersion ~= latestVersionInfo.Version then
			notify('Outdated','Get the new version at the discord')
		end
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, PhantomOS may have issues or features that are not functioning correctly.") end
end)
